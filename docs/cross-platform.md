...
首次啟動會要求輔助使用權限，首次截圖會要求螢幕錄製權限。目前專案是透過 `uv`／`uvx` 執行，並未封裝成獨立的 macOS App，因此「系統設定 → 隱私權與安全性 → 輔助使用／螢幕錄製」中的授權對象通常不會顯示為 FShot，而是啟動 `uv` 指令所在的 App，例如「終端機」、iTerm2、Warp 或 IDE 的宿主程式。請對實際啟動 FShot 的 App 開啟兩項權限；變更後需完全結束並重新開啟該 App，再重新執行 FShot。未授予輔助使用權限時，控制項選取與全域快捷鍵不可用；視窗 hit-test 仍會 best-effort fallback。
